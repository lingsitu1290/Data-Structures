...
def sort_by_cohort(filename):
    ...
    for line in file_name: 
        ...
        # Code to extract all student's first and last name
        name = full_list[0] + " " + full_list[1]
        # all_students.append(name)

        # find the names of those students in the Winter 2015 and put into Winter_2015 list
        if full_list[4] == "Winter 2015\n":
            winter_15.append(name)
        if full_list[4] == "Spring 2015\n":
            spring_15.append(name)
        if full_list[4] == "Summer 2015\n":
            summer_15.append(name)
        # fin the names of the all TAs, TAs have "I\n" in their cohert semester
        if full_list[4] == "I\n":
            tas.append(name)

    #Combine all lists to find all students
...
def students_by_house(filename):
    ...
    for line in file_name: 
        ...
        if full_list[2] == "Gryffindor":
            gryffindor.append(name)
            gryffindor.sort()
        elif full_list[2] == "Hufflepuff":
            hufflepuff.append(name)
            hufflepuff.sort()
        elif full_list[2] == "Slytherin":
            slytherin.append(name)
            slytherin.sort()
        elif full_list[2] == "Dumbledore's Army":
            dumbledores_army.append(name)
            dumbledores_army.sort()
        elif full_list[2] == "Order of the Phoenix":
            order_of_the_phoenix.append(name)
            order_of_the_phoenix.sort()
        elif full_list[2] == "Ravenclaw":
    ...
    # All all sorted house lists into all_students lists 
...
def all_students_tuple_list(filename):
    ...
    for line in file_name: 
        full_list =line.split('|')
        #create each student tuple
        student_tuple = (full_list[0], full_list[1], full_list[2], full_list[3], full_list[4])
        #append each student tuple to the student_list
